...
## アーキテクチャ
このプロジェクトは、ゲーム内部で動作する3つのモジュールと、2つの外部コントロールパネルの計5つのコンポーネントで構成されています。
...
5. **Live Dashboard** (React + FastAPI): リアルタイムの可視化ダッシュボードです。FastAPI バックエンド (`live_ui_server.py`) と React フロントエンド (`live_ui_web/`) で構成されています。
...
### 4. Live Dashboard の起動 (任意)
リアルタイムの React ダッシュボードを使用する場合：
```bash
# バックエンドの起動
cd R-NaD
source venv/bin/activate
...
npm run dev
```
*ダッシュボードは `http://localhost:5173` で利用可能です。*

### 5. R-NaD の学習実行 (Optional)
